...
class AWSLambdaBackend:
    ...
    def _get_default_runtime_name(self):
        ...
        return f'lithops-custom-image-runtime-v{py_version}'

    def _is_container_runtime(self, runtime_name):
        name = runtime_name.split('/', 1)[-1]
        return 'lithops-custom-image-runtime-v' not in name
        ...
        try:
            ...
            if not images:
                raise Exception(f'Runtime {runtime_name} is not present in ECR.'
                                'Consider running "lithops runtime build -b aws_lambda_custom_image ..."')
    ...
    def delete_runtime(self, runtime_name, runtime_memory, version=__version__):
        ...
        # layer_builder_func_name = '_'.join([self.package, 'layer_builder_512MB'])
        # self.lambda_client.delete_function(FunctionName=layer_builder_func_name)
