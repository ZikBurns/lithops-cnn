        ...
        for i, task_executor in enumerate(self.task_executors):
            if i>0:
                ...
                if task_executor.num_cpus:
                    if previous_executor.output_queue.isQueue():
                        task_executor.input_queue = FlexQueue("InputPipeQueue", task_executor.num_cpus-1)
                        previous_executor.output_queue = task_executor.input_queue
                    else:
                        task_executor.input_queue = previous_executor.output_queue
                    task_executor.output_queue = FlexQueue("OutputPipeQueue", task_executor.num_cpus-1)
                else:
                    task_executor.input_queue = previous_executor.output_queue
                    task_executor.output_queue = FlexQueue()
                ...
                if task_executor.num_cpus:
                    ...
                    task_executor.input_queue = FlexQueue()
                    task_executor.output_queue = FlexQueue()
